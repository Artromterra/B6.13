...
def albums(artist):
    ...
    if not albums_list:
        ...
        result = "Список из {} альбомов".format(len(albums_list)) + " " + "{}:\n".format(artist)
        result += "\n".join(album_names)
    return result
...
def create_album():
    ...
    try:
        ...
        print("Новый #{} альбом успешно сохранен".format(new_album.id))
        result = "Альбом под №{} успешно сохранен".format(new_album.id)
    return result
